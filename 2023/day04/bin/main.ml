...
let pow2 n =
  List.init n Fun.id
  |> List.fold_left (fun acc _ -> if acc = 0 then 1 else 2 * acc) 0
...
let parse_line line =
  ...
  let my_nums =
    nth 0 x |> split_str " " |> List.map int_of_string_opt
    |> List.filter_map Fun.id
  in
  let win_nums =
    nth 1 x |> split_str " " |> List.map int_of_string_opt
    |> List.filter_map Fun.id
  in
  let winning_nums = List.filter (is_in win_nums) my_nums in
  winning_nums |> List.length

let solve lines = lines |> List.map parse_line |> List.map pow2 |> sum
